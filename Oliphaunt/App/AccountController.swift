...
//                )
    ...
    private func processOAuthCallback(
        ...
        try await createSession(token: token, client: client, animated: true)
    }
    ...
    func autoLogin() async {
        ...
        guard let url = URL(string: "https://\(lastUsedServer)") else { return }

        let client = Client(baseURL: url)

        guard
            let token = keychain.decodeValue(
                Token.self,
                for: KeychainScope.user(identifier: lastUsedUserID).key(for: url)
            ) else {
            return
        }

        try? await createSession(token: token, client: client, animated: false)
    }

    @MainActor
    private func createSession(token: Token, client: Client, animated: Bool) async throws {
        let manfredAccount = try await client.send(Accounts.me(accessToken: token.accessToken))
        withAnimation(animated ? .oliphaunt : .none) {
            account = Account(manfredAccount: manfredAccount)
            session = .init(accessToken: token.accessToken, client: client)
        }
    }
...
enum KeychainScope {
    ...
    func key(for baseURL: URL) -> String {
        ...
        return "oliphaunt:\(rawValue):\(host.lowercased())"
    }
}
